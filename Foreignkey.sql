...
-- Error Code: 1822. Failed to add the foreign key constraint. Missing index for constraint 'addresses2_ibfk_1' in the referenced table 'users'
CREATE TABLE users99 (
...
CREATE TABLE addresses3 (
    ...
    state VARCHAR(100),
    pincode VARCHAR(10),
    CONSTRAINT fk_user FOREIGN KEY (user_id) REFERENCES users99(id)
    -- FOREIGN KEY (user_id) REFERENCES users99(id)
);
...
ALTER TABLE addresses3
ADD CONSTRAINT fk_user FOREIGN KEY (user_id) REFERENCES users99(id);

DROP table addresses3 ;
...
CREATE TABLE addresses3 (
    ...
    state VARCHAR(100),
    pincode VARCHAR(10),
    CONSTRAINT fk_user FOREIGN KEY (user_id) REFERENCES users99(id) ON DELETE CASCADE
);

-- Or alter it later:
ALTER TABLE addresses3
ADD CONSTRAINT fk_user FOREIGN KEY (user_id) REFERENCES users99(id) ON DELETE CASCADE;
-- Error Code: 1826. Duplicate foreign key constraint name 'fk_user'  
